            ...
            let dataTable = $('#shipment-table-').DataTable({
                ...
                },
                {
                    ...
                    data: function (data, type, dataToSet) {
                        var editUrl = "{{ route('shipments.edit', ':id') }}".replace(':id', data.id);
                        var deleteUrl = "{{ route('shipments.destroy', ':id') }}".replace(':id', data.id);
                        var addBoxUrl = "{{ route('shipments.boxes.create', ':id') }}".replace(':id', data.id);
                        var manageBoxesUrl = "{{ route('shipments.boxes.index', ':id') }}".replace(':id', data.id);
                        ...
                        return `<div class="dropdown d-flex">` +
                            ...
                            `@lang('Edit')` +
                            `</x-dropdown-item>` +
                            `<x-dropdown-item href="${manageBoxesUrl}">`+
                            `<x-heroicon-o-list-bullet class="hero-icon-sm me-2 text-gray-400" />`+
                            `@lang('Manage Shipment Boxes')`+
                            `</x-dropdown-item>` +
